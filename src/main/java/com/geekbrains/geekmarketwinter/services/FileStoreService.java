...
public class FileStoreService implements IFileStoreService {
    ...
    public String deleteFile(String fileName) throws IOException {
        int fileCount = fileMetaProvider.getFileCountByName(fileName);
        String hashFile = fileMetaProvider.getHashByFileName(fileName);

        String fName = hashFile + "." + FilenameUtils.getExtension(fileName);

        // Файл нужно удалить физичсески, если на него только одна ссылка
        if (fileCount == 1) {
            systemProvider.deleteFile(fName);
        }

        // Файл из базы данных удаляем в любом случае
        if (!fileName.isEmpty()) {
            fileMetaProvider.deleteByFileName(fileName);
        }
